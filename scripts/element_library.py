...
def condense(K, F, dof_cnt):
    Krr = K[:dof_cnt, :dof_cnt]
    Krc = K[:dof_cnt, dof_cnt:]
    Kcc_inv = np.linalg.inv(K[dof_cnt:, dof_cnt:])
    Fr = F[:dof_cnt]
    Fc = F[dof_cnt:]
    return Krr - np.dot(np.dot(Krc, Kcc_inv), Krc.T), Fr - np.dot(np.dot(Krc, Kcc_inv), Fc)
...
class QuadBilinearIncompatible:
    def __init__(self, nodes, coordinates, dofs, integration):
        self.nodes = nodes
        self.coordinates = coordinates.flatten()
        self.dofs = dofs.flatten()
